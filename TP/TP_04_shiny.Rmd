...
**Note**: Par défaut, lorsque plusieurs histogramme sont tracés, `ggplot` les
...
Un autre [exemple](https://walkerke.shinyapps.io/neighborhood_diversity) 
plus complet présente un jeu de données sur la mixité sociale
...
Pour le voir, créez simplement un nouveau projet shiny, dans le menu:
```
...
(par défaut) ou de la densité (`geom_histogram(aes(y = ..density..), ...)`).
