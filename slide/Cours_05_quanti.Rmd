...
## Fréquences cumulées croissantes {.build}
...
freq <- ggplot(couples,
  ...
  scale_y_continuous(name = "Effectif",
                     sec.axis = sec_axis(trans = ~./ sum(effectifs), name="Fréquence")) + 
  ggtitle("Polygone des fréquences") + xlab("Nombre d'enfants") + theme_minimal()


cumfreq <- ggplot(couples,
                  aes(x = enfants,
                      y = effectif,
                      group = 1)) +
  stat_ecdf() + 
...
ggplot(Titanic, aes(x = Age)) +  ## Age en abscisse
  geom_histogram(                ## Histogramme
    aes(y = ..density..),        ## Densité à la place des comptages
    binwidth = 5,                ## Largeur des classes
    center = 2.5                 ## Centre des classes
  )
```
...
<!-- ```{r, fig.height=3, fig.width=6, fig.align='center', echo = TRUE} -->
<!-- ggplot(Titanic, aes(Age)) + -->
<!--   geom_histogram(aes(y = ..density..), binwidth = 5, center = 2.5) +  -->
<!--   geom_density() -->
<!-- ``` -->
...
ggplot(montpellier, aes(temperature)) +
  geom_histogram(aes(y = ..density..), binwidth = 1, center = 0.5, alpha = 0.7 ) + 
...
Médiane : `r round(median(montpellier$temperature), 2)`°C
...
**Quartile à 0.25** :
Point tel que $25\%$ des données sont à droite, et $75\%$ à gauche.
...
**Quartile à 0.75**:
Point tel que $75\%$ des données sont à droite, et $25\%$ à gauche.
...
ggplot(montpellier, aes(y = temperature)) +
 geom_point(aes(x = jit), color="black") +  
  # ggtitle("Variance") +
  geom_hline(yintercept = mean(montpellier$temperature), color = "blue", size = 1.5) + 
...
**Variance**: `r round(var(montpellier3$temperature), 2)` $(°C)^2$

**Écart-type**: `r round(sd(montpellier3$temperature), 2)` $°C$
...
**Écart inter-quantile** de niveau $\beta$ ($0 < \beta < 1$):
$$
