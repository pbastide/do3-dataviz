...
```{r penguin-agg, message = FALSE}
library(dplyr)
penguins %>% group_by(species) %>% 
  summarise(effectif = n(),
            frequence = n() / length(species))
```
...
side <- ggplot(penguins_nona,
  ...
  geom_bar() + xlab(NULL) + ylab("effectif") + scale_fill_discrete(guide = FALSE) + theme_minimal()
...
pie <- ggplot(pen_sum,
  ...
  geom_arc_bar(stat = "pie") + scale_fill_discrete(guide = FALSE) + theme_void()
...
## Parts de marché - Camenbert {.build}
